...
- [x] **Seedデータ生成 (v0.1.0)**: 15シナリオ/125イベントの初期fixtureを作成。 (2026-03-05)
  ...
  - Current fixture version: `v0.6.0` (2026-03-20: LLM fault scenarios for malformed JSON / context length exceeded / token limit)
...
- [x] **依存障害シナリオ**: Redis timeout / Gemini 429・5xx / DB接続劣化をベンチシナリオとして実装。 (2026-03-19: `backend/testbench_runner.py`, `tests/test_testbench_runner.py`, `scripts/generate_testbench_dataset.py`, `tests/fixtures/testbench/*`)
- [x] **LLM固有障害シナリオ**: malformed JSON（スキーマ崩れ）/ context length exceeded / token limit 超過時のフォールバック挙動を検証。 (2026-03-20: `backend/testbench_runner.py`, `scripts/generate_testbench_dataset.py`, `tests/fixtures/testbench/*`, `tests/test_testbench_runner.py`)
- [x] **メトリクス収集**: シナリオ単位で `request_count`, `error_rate`, `p50/p95/p99`, `state_drift_count` を記録。 (2026-03-21: `backend/testbench_runner.py`, `tests/test_testbench_runner.py`)
- [x] **失敗時証跡**: 失敗イベントの triggered_rules, state snapshot, observed_state_path を quality gate failure の `trace` フィールドとして自動保存。 (2026-03-21: `backend/testbench_runner.py`, `tests/test_testbench_runner.py`)
- [x] **差分比較レポート**: 前回runとの差分（失敗増減、遅延悪化、誤検知率変化）を Markdown で出力。 (2026-03-21: `backend/testbench_runner.py`, `tests/test_testbench_runner.py`)

### 1.6.6 受け入れ基準 (POC運用)
- [x] **機能基準**: 18シナリオ全件をRunnerで再生し、期待ゲート判定を自動実施できる。 (2026-03-21: local regression 18/18 passed, all quality gates green)
- [x] **安定性基準**: 同条件3連続実行で判定ぶれが 0 件である。 (2026-03-21: local-verify-{1,2,3} 各 18/18 passed, failure_count=0)
- [x] **運用基準**: 定期実行・失敗通知・レポート保存が手動介入なしで回る。 (2026-03-21: CI workflows — smoke/PR, regression/daily, soak/weekly — artifacts + JUnit report 保存済み)
- [x] **継続改善基準**: 新規不正パターン追加時、fixture更新 -> ベンチ実行 -> レポート反映が1PRで完了できる。 (2026-03-21: dataset_version changelog + generate_testbench_dataset.py + CI pipeline で実証済み)
...
- [x] **Docker**: `Dockerfile` の最適化（Multi-stage build） (2026-03-27: root `Dockerfile`, `.dockerignore`, `README.md`)
