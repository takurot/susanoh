...
- [x] **Soak (週次)**: 長時間リプレイでメモリリーク/状態不整合/遅延悪化を検知。 (2026-03-11: `.github/workflows/testbench-soak.yml`, `backend/testbench_runner.py`, `tests/test_testbench_runner.py`, `docs/TESTBENCH_POLICY.md`)
  - Runner 内ループで namespaced scenario replay を反復し、per-scenario の `state_drift_count` と iteration latency を artifact に集約する。
  - Soak summary に `peak_rss_mb` / `peak_rss_growth_mb` を含め、長時間実行時のメモリ増分を観測可能化。
  - `SUSANOH_TESTBENCH_SOAK_ITERATIONS` repository variable または `--soak-iterations` で replay 回数を調整可能。
...
- [x] **Seedデータ生成 (v0.1.0)**: 15シナリオ/125イベントの初期fixtureを作成。 (2026-03-05)
  ...
  - Current fixture version: `v0.1.1` (2026-03-10: regression fault injection metadata + expected-value refresh)
