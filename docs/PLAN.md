...
- [ ] **Regression (日次)**: 全シナリオ + fault injection を20-40分で実行し、日次サマリを保存。
  - Status: 2026-03-09 に `.github/workflows/testbench-regression.yml` と `.github/workflows/testbench-regression-live.yml` を追加し、日次/週次スケジュールと artifact 保存の土台を整備。fault injection シナリオ統合は継続。
