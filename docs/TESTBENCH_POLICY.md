...
- 縮小した `Regression-Live` は `.github/workflows/testbench-regression-live.yml` で毎週月曜 `05:00 JST` (`20:00 UTC` 日曜) に staging secrets 利用時のみ実行し、staging が API key 必須構成なら `SUSANOH_TESTBENCH_STAGING_API_KEY` も前提に含めて実 API 健全性を別レーンで確認する。`live` モードの artifact には `backend.live_api_verification` と連動した `Live Verification` セクションも含まれ、Gemini などの外部依存の生存確認を `summary.json` / `report.md` / `junit.xml` に同居させる。
