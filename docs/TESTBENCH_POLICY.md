...
- 週次 Soak は `.github/workflows/testbench-soak.yml` で毎週日曜 `04:00 JST` (`19:00 UTC` 土曜) に実行し、scenario-level の namespaced replay を繰り返して長時間実行時の latency 劣化と state drift を集計する。artifact には `peak_rss_mb` / `peak_rss_growth_mb` も含め、必要に応じて `SUSANOH_TESTBENCH_SOAK_ITERATIONS` repository variable で replay 回数を上書きできる。
