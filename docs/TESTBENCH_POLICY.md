...
- 日次 Regression は `.github/workflows/testbench-regression.yml` で毎日 `03:00 JST` (`18:00 UTC`) に実行し、`local` プロファイルの決定論的 L2 で flakiness とコストを抑える。
- 縮小した `Regression-Live` は `.github/workflows/testbench-regression-live.yml` で毎週月曜 `05:00 JST` (`20:00 UTC` 日曜) に staging secrets 利用時のみ実行し、実 API 健全性を別レーンで確認する。
