# 機能仕様書：AI駆動型 経済圏防衛ミドルウェア「Susanoh」

## 0. 概要

「Susanoh（スサノヲ）」は、オンラインゲーム経済圏における不正取引（RMT・資金洗浄・Bot活動）を検知・隔離するためのセキュリティミドルウェアです。
ゲームサーバーから送信されるイベントログをリアルタイムで解析し、**誤検知によるユーザー離脱を最小限に抑えつつ、不正な資金移動のみを的確に遮断**します。
...
- **導入の容易性**: ゲームサーバー側はREST APIへのイベント送信のみで連携可能。

---

## 1. システムアーキテクチャ

Susanohは、以下のコンポーネントで構成される分散システムです。
...
| **State Store** | Redis | ユーザー状態（ステートマシン）、直近イベントウィンドウの高速管理 |
...
| **Primary Database** | PostgreSQL | ユーザープロファイル、監査ログ、分析結果の永続化 |
...
- CI/CDパイプラインの整備
- コンテナオーケストレーション（K8s manifest）

### Phase 3: パフォーマンス最適化・機能拡張
- **Rust Gateway**: L1エンジンの一部をRustで再実装し、超低レイテンシ化。
- **Multi-tenancy**: 複数ゲームタイトルの統合管理対応。
- **SDK**: Unity/Unreal Engine向けクライアントSDKの提供。
